...
action :add do
  ...
  # Write out the file
  repo_file = template "/etc/yum.repos.d/#{new_resource.repo_name}.repo" do
    cookbook "yum"
    source "repo.erb"
    mode "0644"
    variables({
  ...
  if repo_file.updated_by_last_action?
    Chef::Log.info "Added #{new_resource.repo_name} repository to /etc/yum.repos.d/#{new_resource.repo_name}.repo"
